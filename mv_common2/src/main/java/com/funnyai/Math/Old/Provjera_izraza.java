...
final public class Provjera_izraza
   ...
   // provjera : da
   public static boolean pokretanje( Vector tokeni, Raspon raspon_greske,
   ...
   // provjera : da
	private static boolean zarez_dobar( Vector tokeni, int poz )
		{
		int		i=0;
		Token		t;
		...
		if ( poz<=0 )
			...
			{
			t=(Token)tokeni.elementAt( poz-1 );
			if ( t.oznaka!='F' )
				return false;
			}
		return true;
   ...
   // provjera : da
   private static boolean provjera_zareza( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
		{
      Token t;
      boolean povrat=true;
	...
	// provjera : da
	public static int kraj_funkcije( Vector tokeni, int pozicija_funkcije )
		{
	...
	// provjera : da
	private static int broj_argumenata_funkcije( Vector tokeni, int pozicija_funkcije )
		{
		int		i=pozicija_funkcije+2;
		int		broj_zareza=0;
		Token		t;
	...
	// provjera : da
	private static boolean provjera_funkcija( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
		{
		int			i;
		Function		fn;
	...
	// provjera : da
	private static boolean provjera_redoslijeda( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
		{
		boolean			povrat=true;
		Token				t1;
		...
		for ( int i=0; i<tokeni.size()-1; i++ )
			{
			t1=(Token)tokeni.elementAt( i );
			t2=(Token)tokeni.elementAt( i+1 );
	...
	// provjera : da
	private static boolean provjera_pocetka_izraza( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
		{
		char 		oznaka;
		Token		t;

		t=(Token)tokeni.elementAt( 0 );
		oznaka=t.oznaka;
	...
	// ako ne, vraca false i puni raspon_grekse i tekst_greske
	// provjera : da
	private static boolean provjera_kraja_izraza( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
		{
		char 		oznaka;
		Token		t;

		t=(Token)tokeni.elementAt( tokeni.size()-1 );
		oznaka=t.oznaka;
   ...
   // i puni raspon_greske i tekst_greske, inace vraca true
	// provjera : da
	private static boolean provjera_ispravnosti( Vector tokeni, Raspon raspon_greske,
		StringBuffer tekst_greske )
    	{
      boolean	greska=false;
		...
		for ( pocetak=0; pocetak<tokeni.size(); pocetak++ )
			{
			t=(Token)tokeni.elementAt( pocetak );
			if ( t.oznaka=='G' )
				{
				greska=true;
				break;
		...
		if ( greska )
			...
			for ( kraj=pocetak+1; kraj<tokeni.size()
				&&	((Token)(tokeni.elementAt( kraj ))).oznaka=='G'; kraj++ );

			t = (Token)tokeni.elementAt( pocetak );
			pocetak=t.pozicija;
			t = (Token)tokeni.elementAt( kraj-1 );
			kraj=t.pozicija;
