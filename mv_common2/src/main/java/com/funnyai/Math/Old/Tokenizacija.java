...
final public class Tokenizacija
   ...
   public static double string_to_double( String s )
	...
	// provjera	: da
	public static Varijabla varijabla( String izraz, Vector varijable )
		{
		int			i;
		...
		for ( i=0; i<varijable.size(); i++ )
			{
			v=(Varijabla)varijable.elementAt(i);
	...
	// provjera	: da
	public static Function funkcija( String izraz, Vector funkcije )
		{
		int			i;
		int			duzina_funkcije;
		Function		f;
		...
		for ( i=0; i<funkcije.size(); i++ )
			{
			f=(Function)funkcije.elementAt(i);
	...
	// provjera	: da
	public static Operator operator( String izraz, Vector operatori )
		{
		int			i;
		...
		for ( i=0; i<operatori.size(); i++ )
			{
			o=(Operator)operatori.elementAt(i);
	...
	//	拆卸的表达
	public static Vector rastavljanje_izraza(
			String s, 
			Vector varijable,
			Vector operatori, 
			Vector funkcije )
		{
		Vector			tokeni = new Vector();
		int				duzina;
		...
		while ( !s.equals("") )
         ...
         pozicija=pozicija+( s.length() - s.trim().length() );
			...
			if ( duzina>0 )
				...
				tokeni.addElement( new Token( dio, 'H', pozicija, duzina ) );
			...
			if ( duzina>0 )
				{
				String dio = s.substring( 0, duzina );
				tokeni.addElement( new Token( dio, 'O', pozicija, duzina ) );
				s=s.substring(duzina);
				zastavica=true;
				pozicija=pozicija+duzina;
				continue;
			...
			if ( duzina>0 )
				{
				String dio = s.substring( 0, duzina );
				tokeni.addElement( new Token( dio, 'B', pozicija, duzina ) );
				s=s.substring(duzina);
				zastavica=true;
				pozicija=pozicija+duzina;
				continue;
            ...
            tokeni.addElement( new Token( broj, 'D', pozicija, duzina ) );
			...
			// VARIJABLA
			Varijabla v = Tokenizacija.varijabla( s, varijable );
			if ( v!=null )
				{
				duzina=v.varijabla.length();
				tokeni.addElement( new Token( v, 'V', pozicija, duzina ) );
				s=s.substring(duzina);
				zastavica=true;
				pozicija=pozicija+duzina;
				continue;
			...
			if ( f!=null )
				...
				tokeni.addElement( new Token( f, 'F', pozicija, duzina ) );
				s=s.substring(duzina);
				zastavica=true;
				pozicija=pozicija+duzina;
				continue;
			...
			if ( o!=null )
				...
				tokeni.addElement( new Token( o, 'P', pozicija, duzina ) );
				s=s.substring(duzina);
				zastavica=false;
				pozicija=pozicija+duzina;
				continue;
			...
			if ( status )
				...
				tokeni.addElement( new Token( dio, '(', pozicija, 1 ) );
				s=s.substring( 1 );
				zastavica=false;
				pozicija=pozicija+1;
				continue;
			...
			if ( s.charAt( 0 )==',' )
				{
				String dio = ",";
				tokeni.addElement( new Token( dio, 'Z', pozicija, 1 ) );
				s=s.substring( 1 );
				zastavica=false;
				pozicija=pozicija+1;
				continue;
			...
			tokeni.addElement( new Token( s.substring( 0, 1 ), 'G', pozicija, 1 ) );
			zastavica=false;
			s=s.substring( 1 );
			pozicija=pozicija+1;
