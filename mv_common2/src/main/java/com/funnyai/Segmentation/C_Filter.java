...
public class C_Filter {
	...
	public void loadFilter(){
		...
		for (int i=0;i<strFiles.length;i++){
			System.out.println(C_Filter.class.getResource("/").toString());
			System.out.println("File:"+strFiles[i]);
			InputStream url=C_Filter.class.getResourceAsStream("/data/"+strFiles[i]);
			try {
				loadFilterFile(url);
			} catch (IOException e) {
				e.printStackTrace();
...
//            
//            
//	    String strLeft= strLine.substring(0,1);
//	    C_Word_Convert pWord;
...
//	    }
	...
	public String Filter(String strInput)
	{
	    ...
	    String strReturn = "";
	    for (int i = 0; i < strSplit.length; i++) {
	        if (this.ifHasThisWord(strSplit[i]) == false) {
	            strReturn += strSplit[i] + " ";
	        }
	    }
