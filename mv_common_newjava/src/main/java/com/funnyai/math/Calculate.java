...
final public class Calculate {//Izracunavanje
    ...
    public static int pozicija_prve_zatvorene_zagrade(ArrayList<Token> tokeni) {
        Token t;
    ...
    // provjera : da
    public static int pozicija_otvorene_zagrade(
            ArrayList<Token> tokeni, int pozicija_zatvorene_zagrade) {
        int i;
        Token t;
    ...
    // provjera : da
    static public Raspon raspon_zagrada(ArrayList<Token> tokeni) {
    ...
    // provjera : da
    public static String spoj_tokena(ArrayList<Token> tokeni) {
    ...
    // provjera : da
    public static int pozicija_operatora(ArrayList<Token> tokeni, Raspon raspon_zagrada) {
    ...
    public static double rezultat_operacije(double broj1, double broj2, String op) {
        ...
        if (op.equals("^") || op.equals("**")) // rezultat=Math.pow( broj1, broj2 );
        {
            rezultat = Matematika.potenciranje(broj1, broj2);
        } else if (op.equals("*")) // rezultat=broj1*broj2;
        {
    ...
    public static double vrijednost_operanda(Token t) {
        if (t.oznaka == 'V') {
            return ((Variable) t.token).vrijednost;
        } else if (t.oznaka == 'D') {
    ...
    // provjera : da
    public static boolean korak_izracunavanja(ArrayList<Token> tokeni) {
        ...
        t = (Token) tokeni.get(poz_max_op - 1);
    ...
    // provjera : da
    private static void izbaci_tokene(ArrayList<Token> tokeni, int pozicija_funkcije) {
        while (((Token) tokeni.get(pozicija_funkcije)).oznaka != ')') {
            tokeni.remove(pozicija_funkcije);
        }
    }

    public static void izbacivanje_visestrukih_zagrada(ArrayList<Token> tokeni) {
        ArrayList<Token> pomocni = (ArrayList<Token>) tokeni.clone();// tokeni.clone();
        Raspon r;

        while ((r = raspon_zagrada(pomocni)) != null) {
            if (r.pocetak != 0 && r.kraj != pomocni.size() - 1
                    && ((Token) pomocni.get(r.pocetak - 1)).oznaka == '('
                    && ((Token) pomocni.get(r.kraj + 1)).oznaka == ')') {
                pomocni.remove(r.kraj + 1);
                pomocni.remove(r.pocetak - 1);
                tokeni.remove(r.kraj + 1);
                tokeni.remove(r.pocetak - 1);
            } else // izbacivanje vanjskih zagrada
            if (r.pocetak == 0 && r.kraj == tokeni.size() - 1) {
                tokeni.remove(r.kraj);
                tokeni.remove(r.pocetak);
            } else {
                // dummy tokeni na mjestu zagrada
                pomocni.set(r.kraj,new Token(null, (char) 0, 0, 0));
                pomocni.set(r.pocetak,new Token(null, (char) 0, 0, 0));
            }
        }
        return;
    }

    private static void izbacivanje_zagrada(ArrayList<Token> tokeni, int poz) {
        if (poz > 1
                && ((Token) tokeni.get(poz - 2)).oznaka != 'F'
                && ((Token) tokeni.get(poz - 1)).oznaka == '('
                && ((Token) tokeni.get(poz + 1)).oznaka == ')'
                || poz == 1
                && ((Token) tokeni.get(0)).oznaka == '('
                && ((Token) tokeni.get(2)).oznaka == ')') {
            tokeni.remove(poz + 1);
            tokeni.remove(poz - 1);
        }
...
//      }
    ...
    private static double result_function(
            ArrayList<Token> tokeni, int pozicija_funkcije) {
        double rezultat = 0;
        double operand;
        Token t;
        String fn;

        t = (Token) tokeni.get(pozicija_funkcije);
        fn = ((Function) t.token).funkcija;

        t = (Token) tokeni.get(pozicija_funkcije + 2);
        ...
        if (fn.equals("sin")) {
            ...
            t = (Token) tokeni.get(pozicija_funkcije + 4);
    ...
    // provjera : da
    //结果
    public static ArrayList rezultat(ArrayList<Token> tokeni) {
        ...
        Token t = (Token) tokeni.get(0);
        if (t.oznaka != 'D') {
            double x = vrijednost_operanda(t);
            tokeni.set(0,new Token(new Double(x), 'D', 0, 0));
            v.set(0,spoj_tokena(tokeni));
        }
